...
RSpec.describe User, type: :model do
  ...
  it 'まず初めに、依存してるクラスのnewに引数を入れない場合を考える' do
    ...
    # ちなみに、and_returnの代わりにand_call_originalを使用すると下記のエラーが発生する
    # #<InstanceDouble(ExternalApi) (anonymous)> is a pure test double. `and_call_original` is only available on a partial double.
    # allow(moc).to receive(:fetch).and_call_original

    index

    expect(ExternalApi).to have_received(:new).once
  end

  it 'allowを使うとpartial doubleになるので、シンプルに書ける' do
    allow(ExternalApi).to receive(:new).and_call_original

    expect(index).to eq('hello')
  end
